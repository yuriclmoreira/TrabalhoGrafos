...
list<string> Dijkstra::caminhoMinimo(Grafo *grafo, int noI, int noAlvo){
    Vertices* ver =  grafo->procurarNo(noAlvo);
    Vertices* inicial = grafo->procurarNo(noI);
    if(ver->getGrauEntrada() == 0){ // Se o vertice alvo n tem grau de entrada n tem caminho ate ele
        cout << endl << "Nao existe caminho entre o vertice " << inicial->getNome() << " e o vertice " << ver->getNome() << endl;
        return caminho; 
    }
    ...
    if(dist[noAlvo]<INT_MAX/2){ // se houve caminho encontrado
        criaCaminho(grafo, noAlvo, ant, noI); // cria lista do menor caminho encontrado
...
void Dijkstra::criaCaminho(Grafo* grafo,int noA, int ant[], int noI){ // Busca nos predecessores para fazer o caminho
    Vertices* auxInicial = grafo->procurarNo(noI);
    Vertices* auxAlvo = grafo->procurarNo(noA);
    if(noA != noI){ // vertice selecionado diferente do vertice inicial
        caminho.push_front(auxAlvo->getNome()); // puxa vertice para o comeco
        int aux = ant[noA];
        criaCaminho(grafo, aux, ant, noI); // passa anterior do vertice recursivamente
    }
    else // vertice selecionado é o vertice inicial
        caminho.push_front(auxInicial->getNome()); // puxa vertice inicial para comeco do caminho
}